...
function nvidia_config() {
    ...
    # Create full NVIDIA kernel module configuration with detailed comments
    exec_log "sudo bash -c 'cat > $nvidia_config_file <<EOF
#
...
options nvidia NVreg_RegistryDwords=\"PowerMizerLevel=0x3\"
options nvidia NVreg_RegistryDwords=\"PowerMizerDefault=0x3\"
options nvidia NVreg_RegistryDwords=\"PowerMizerDefaultAC=0x3\"
EOF
...
function nvidia_runtime_pm_udev_rule() {
    ...
    exec_log "sudo bash -c 'cat > $udev_file <<EOF
...
# Create external script for initramfs regeneration
function create_initramfs_regen_script() {
    local script_path="/usr/local/bin/nvidia-regenerate-initramfs.sh"
    if [ -f "$script_path" ]; then
        exec_log "sudo rm $script_path" "Removing existing initramfs regeneration script"
    fi
    exec_log "sudo bash -c 'cat > $script_path <<EOF
#!/bin/sh
...
EOF
'" "Creating initramfs regeneration script"
    exec_log "sudo chmod +x $script_path" "Making the script executable"
}
...
function create_pacman_hook() {
    ...
    # Create the new hook
    exec_log "sudo bash -c 'cat > $hook_file <<EOF
[Trigger]
Operation=Install
Operation=Upgrade
Operation=Remove
...
NeedsTargets
Exec=/usr/local/bin/nvidia-regenerate-initramfs.sh
EOF
...
function nvidia_drivers() {
    ...
    # Create external initramfs regeneration script
    create_initramfs_regen_script
